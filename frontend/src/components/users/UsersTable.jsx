...
export default function UsersTable() {
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(term);
    const filtered = usersData.filter(user => user.firstname.toLowerCase().includes(term) || user.lastname.toLowerCase().includes(term) || user.email.toLowerCase().includes(term));
  ...
  return (
    ...
    > 
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-black">Liste des utilisateurs</h2>
            ...
            {filteredUsers?.map((user) => (
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-black-400 flex gap-2 items-center truncate">
                  <img src={user?.avatar} className='size-10 rounded-full object-cover' />
                  {`${user?.firstname} ${user?.lastname}`}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-black-400">
                  {user?.email}
                </td>
                <td className={"px-6 py-4 whitespace-nowrap"}>
                  <span className="px-2 text-xs inline-flex leading-5 font-semibold text-blue-50 rounded-full bg-blue-800">
                    {user?.role === 2584 
                      ? "Admin"
                      : user?.role === 4181 
                        ? "User"
                ...
                <td className="px-6 py-4 whitespace-nowrap ">
                  <span className={`px-2 text-xs inline-flex leading-5 font-semibold text-blue-50 rounded-full ${
                    user?.status === "Actif" || true
